...
# ⚠️ YOUR BOT TOKEN FROM BOTFATHER
...
def main():
    ...
    # Render Environment Setup for Webhook
    PORT = int(os.environ.get("PORT", 8080))
    
    print(f"🤖 Bot is successfully running with Webhook on port {PORT}...")
    
    application.run_webhook(
        listen="0.0.0.0",
        port=PORT,
        webhook_url=f"https://telegram-media-bot-2-o4qr.onrender.com/{BOT_TOKEN}"
    )

if __name__ == '__main__':
    # Render पर 'asyncio.run()' सीधा चलाने से 'no current event loop' आ सकता है,
    # इसलिए हम मैन्युअली नया लूप सेट कर रहे हैं।
    try:
